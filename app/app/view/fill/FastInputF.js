// * кдасс для быстрого ввода ставок
Ext.define('Office.view.fill.FastInputF', {
    singleton: true,
    alternateClassName: ['FastInputF'],

    outcomes: {
        1: {
            outcome_id: function (slug) {
                if (Util.in_array(slug, UtilMarkets.SPORTS_WITHOUT_DRAW))
                    return 216;
                else
                    return 190;
            },
            outcome_name: function (slug) {
                if (Util.in_array(slug, UtilMarkets.SPORTS_WITHOUT_DRAW))
                    return 'coeff_CW_P1'
                else
                    return 'coeff_FT_1'
